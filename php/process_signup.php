...
/************ SEARCH ***********/
$con = connectTo();
$exists = $con->query("select * from `db_sams`.`teacher` where email = '$email'");
if($exists && $con->affected_rows) {
  $con->close();
  respond("error","exists");
} 
/*********** INSERT ************/
$addTeacher = $con->query("insert into `db_sams`.`teacher` (`name`, `email`, `phone`, `password`) values ('$name', '$email', '$phone', '$pass')");
